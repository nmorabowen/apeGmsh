...
| **B1** `element="beam"` rebar | ✅ **DESIGN RESOLVED** (user, 2026-06-21). KEY finding: the cage emits geometry+coupling only; `LadrunoEmbeddedRebar` is coupling-ONLY (no axial stiffness); `RebarMember.element` is metadata never consumed; the structural bar element is user-emitted today. **Decision: cage AUTO-EMITS the structural element behind an opt-in `place(emit_elements=…)` flag** (truss→`CorotTruss`, beam→`dispBeamColumn`), wired via a declare→resolve→emit broker channel mirroring `g.reinforce`. Beam section = **circular fiber from `db` + steel material**. **B1a ✅ SHIPPED** (opt-in `place(emit_elements=True)` → `CorotTruss` per bar line cell via a dedicated `emit_rebar_elements` pass; `RebarElementRecord` carries connectivity resolved from the live mesh at `get_fem_data`, since dim-1 cells are dropped from a dim-3 FEMData; material by name; partitioned + `element="beam"` fail-loud). **B1a.2 ✅ SHIPPED** (neutral-H5 persistence of `rebar_elements` → `/rebar_elements` group, schema 2.16.0, mirrors the A1 `/reinforce_ties` pattern; `to_h5` warning removed; compose preserves the host's). **B1b DESIGN ✅** (all injection points pinned; engineering sub-decisions made — no further human gate): beam auto-emit is **embedded-only** (`ndf=6` needs the bar's own nodes; conformal+beam raises); circular **Fiber `patch circ`** section emitted directly in `emit_rebar_elements` (no new primitive) + **Lobatto** integration + per-segment **`geomTransf`** + **`dispBeamColumn`**; **`ndf=6`** via extending `infer_node_ndf` (`build.py:336`) to bump beam-rebar nodes; **twist folds in** (beam is singular without it — B0.3 `zeroLength`+SP ghost node, no new C++ classtag); ungate `RebarComposite.py:1129`. **Adversarial gate REQUIRED** (core-adjacent). Remaining: **B1b** implementation (+ B3 ghost-tag/H5). Deferred: compose source-Part carry of `rebar_elements`. See `plan_rebar_p5.md` §B1b. |
