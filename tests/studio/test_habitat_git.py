...
def test_gitignore_bulk_hidden_run_json_visible(tmp_path: Path) -> None:
    ...
    # reports/figures/ — the zero-depth (flat) case must keep working too,
    # and png-scoping means a stray binary in a figure folder stays hidden.
    fig_dir = target / "reports" / "figures" / "case-a"
    fig_dir.mkdir(parents=True)
    (fig_dir / "fig.png").write_bytes(b"\x89PNG")
    (fig_dir / "stray.h5").write_bytes(b"\x00" * 16)
    (target / "reports" / "figures" / "flat.png").write_bytes(b"\x89PNG")
