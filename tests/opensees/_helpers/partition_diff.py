...
The third coordinate is **absent in an ``ndm == 2`` deck** — the build
layer trims each ``node`` line to the ``ndm`` coordinates OpenSees
actually consumes, because a trailing ``0.0`` desynchronises the
parser's optional-argument scan and swallows the following ``-ndf K``.
Both regexes make ``<z>`` optional for that reason: a 2-coordinate
foreign decl that stopped matching would not fail here, it would
**silently** re-classify as ordinary content and move the diff.
...
# the partition indent ``_LineBuf`` adds).  ``<z>`` is present only in
# an ``ndm == 3`` deck.  The regex matches the whole line modulo
# leading whitespace.
...
# *coords, "-ndf", K)``.  Strings are repr'd with single quotes
