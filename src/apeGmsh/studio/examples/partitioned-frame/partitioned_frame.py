...
ADR 0095 Amendment 7, S8a example library seed. Adapted from
``examples/partition_frame.py`` — this copy is self-contained (no
imports outside apeGmsh + the standard library) so it can run from
wherever ``studio example copy partitioned-frame`` lands it.
...
ADR 0027 INV-1/INV-2 fires, and emits ``partitioned_frame.tcl`` in the
current directory.

Emit-only: this example needs no solver. ``requires: mesh-only``.

Run:
    python partitioned_frame.py            # emit only
    python partitioned_frame.py --run      # try mpiexec too
    python verify.py
...
def build_partitioned_fem():
    ...
    sess = apeGmsh(model_name="partitioned_frame", verbose=False)
    sess.begin()
    try:
...
def maybe_run_openseesmp(tcl_path: Path) -> None:
    ...
    if not (mpi and osmp):
        print("[runtime] mpiexec / OpenSeesMP not on PATH; skipping.")
        return
...
def main(run: bool = False) -> None:
    ...
    print(f"  diaphragms:    {N_STORIES} "
          "(cross-partition replication likely)")
