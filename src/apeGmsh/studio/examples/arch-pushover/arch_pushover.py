...
``examples/shoebuckle_arch.py`` (PR #990) — this copy is self-contained
(no imports outside apeGmsh + the standard library) so it can run from
wherever ``studio example copy arch-pushover`` lands it.
...
Run this with an OpenSees venv interpreter (it needs an openseespy
build that has ASDSteel1D and the MPCO recorder compiled in). Writes
``shoebuckle.mpco`` and ``model.h5`` to the current working directory —
run it from a scratch directory, then ``verify.py`` in the same
directory:
