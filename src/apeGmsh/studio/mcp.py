...
def build_server() -> Any:
    ...
    mcp = Server(
        ...
        instructions=(
            ...
            "not an iso-clip. "
            "render(session=<results>.viewer-session.json) draws one pane "
            "of a "
            ...
            "a v13 session from the retired window is refused by its "
            "PAYLOAD shape, never by its filename, and is never "
            "renamed. "
            "results_pin(session_snapshot=) pins an ADR 0098 session "
            "snapshot (<results>.viewer-session.json) — stamped in the "
            "ledger "
            "under session_snapshot and COPIED into the pin; a v13 payload "
            "is refused. "
