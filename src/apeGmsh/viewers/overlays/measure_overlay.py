...
When the first entity is captured, a sphere marks its centroid.
When the second is captured, a segment is drawn between the centroids
and a label at the midpoint shows ``|d|`` and the per-axis deltas.
Colour comes from ``THEME.current.measure_color``. The next
``add_entity()`` after that wipes both and starts fresh — so the
viewer never accumulates more than one measurement.
...
class MeasureOverlay:
    ...
    def refresh_theme(self) -> None:
        """Redraw in-flight actors in the active ``measure_color``."""
        n = len(self._points)
        if n == 0:
            return
        self._clear_actors()
    ...
    def _measure_color(self) -> str:
        from ..ui.theme import THEME
        return getattr(THEME.current, "measure_color", "#ffff00")

    def _draw_marker(self) -> None:
        """Sphere at the first captured point."""
