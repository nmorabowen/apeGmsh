...
| [0051](0051-bridge-load-consumption.md) | Bridge load consumption: opt-in `from_model` import (no auto-emit), load **cases** on the geometry vs **patterns** on the bridge, all-nodal, stage-scoped patterns + two-mode no-mixing rule, `WarnUnconsumedModelLoads`, `remove_bc` alias | Proposed (2026-05-31; supersedes ADR 0050 LOAD-1 + §5 emit; masses/constraints import = follow-up round) |
