...
# ---------------------------------------------------------------------------
# MultiLinear — piecewise-linear backbone
# ---------------------------------------------------------------------------
...
class MultiLinear(UniaxialMaterial):
    ...
    OpenSees command::
        ...
        uniaxialMaterial MultiLinear tag u1 f1 u2 f2 [u3 f3 ...]

    Stock OpenSees (``MAT_TAG_MultiLinear`` = 52) — no fork dependency,
    and the Tcl and openseespy paths share the one ``OPS_MultiLinear``
    factory, so the emitted flat argument list is identical on both.

    ``points`` are ``(strain, stress)`` / ``(deformation, force)``
    breakpoints on the POSITIVE branch, strictly increasing in the first
    coordinate. The material is odd-symmetric: the constructor mirrors
    each breakpoint onto the negative branch (`MultiLinear.cpp:109-123`),
    so only positive breakpoints are supplied. The initial stiffness is
    the first segment's secant, ``f1 / u1``.

    The characterised-device use case: a trilinear force–deformation law
    for a yielding splice / structural fuse carried on a
    :class:`~apeGmsh.opensees.element.zero_length.ZeroLength` DOF, where
    the acceptance gate is *reproduce this backbone*. Prefer this over
    :class:`Hysteretic` with the same envelope points whenever the law
    itself is the specification — ``Hysteretic`` is a different OpenSees
    class with its own unloading, pinching and damage rules. Measured:
    on a monotonic push the two are **bit-identical** (worst difference
    ``0.0`` over a 320-step sweep, and ``Hysteretic`` extrapolates on the
    last slope too), so a push-only reproduction gate cannot detect the
    substitution — the class choice is the protection, not the gate.

    .. important::
       Past the LAST breakpoint the backbone **keeps the last slope**: it
       does not cap the force and it does not go flat.
       ``MultiLinear::setTrialStrain`` clamps its branch search to
       ``numSlope - 1`` and then extrapolates from the last breakpoint on
       the last segment's slope (`MultiLinear.cpp:183-186`), so a
       trilinear law rides ``K3`` indefinitely. That is the correct way to
       state "there is no bearing backstop at this rung"; append a final
       near-flat segment if a plateau is what you want.

    .. warning::
       Both validation rules guard measured OpenSees behaviour, not
       taste. A ``(0, 0)`` first breakpoint — which the *official
       OpenSees docs example* opens with — makes the initial slope
       ``s(0)/e(0)`` a ``0.0/0.0`` NaN (`MultiLinear.cpp:113`); the
       origin is implicit, so the first pair must be the first NONZERO
       breakpoint. And a non-monotonic strain list is merely *warned*
       about, with a message that misdescribes its own recovery
       ("Continuing with strain_i+1 = 1.2*strain_1" — the code does no
       such thing, `MultiLinear.cpp:102-107`): construction proceeds on
       the raw values, so equal strains divide by zero and decreasing
       strains yield a negative segment slope (`MultiLinear.cpp:121`).
       Both are refused here at construction instead.

    Parameters
    ----------
    points
        Positive-branch ``(strain, stress)`` breakpoints. At least two
        (the parser refuses a single pair), every strain ``> 0``, strains
        strictly increasing.
    """
    ...
    def __post_init__(self) -> None:
        if len(self.points) < 2:
            raise ValueError(
                f"MultiLinear: need >= 2 breakpoints, got {len(self.points)}."
            )
        if any(
            not isinstance(pt, (tuple, list)) or len(pt) != 2
            for pt in self.points
        ):
            raise ValueError(
                "MultiLinear: points must be a sequence of (strain, stress) "
                f"pairs, got {self.points!r} — a flat list of alternating "
                "values is not accepted."
            )
        us = [u for u, _ in self.points]
        if any(u <= 0 for u in us):
            raise ValueError(
                "MultiLinear: all breakpoint strains must be > 0 — the "
                f"negative branch is generated by symmetry, got {us!r}."
            )
        if any(b <= a for a, b in zip(us, us[1:])):
            raise ValueError(
                f"MultiLinear: breakpoint strains must be STRICTLY "
                f"increasing, got {us!r}."
            )
...
class ElasticPP(UniaxialMaterial):
    """``uniaxialMaterial ElasticPP`` — elastic-perfectly-plastic.

    OpenSees command::

        uniaxialMaterial ElasticPP tag E epsyP <epsyN eps0>

    .. warning::
       ``epsyP`` is a yield **strain**, not a yield force/stress. The
       fork derives the yield force internally as ``fyp = E * epsyP``
    ...
    def __post_init__(self) -> None:
        if self.E <= 0:
            raise ValueError(f"ElasticPP: E must be > 0, got {self.E!r}")
        if self.epsyP <= 0:
            raise ValueError(
        ...
        if self.eps0 is not None and self.epsyN is None:
            raise ValueError(
                "ElasticPP: eps0 requires epsyN to also be given "
                "(the OpenSees positional grammar cannot skip epsyN "
                "and supply only eps0)"
            )

    def _emit(self, emitter: Emitter, tag: int) -> None:
        params: list[float] = [self.E, self.epsyP]
        if self.epsyN is not None:
            params.append(self.epsyN)
        if self.eps0 is not None:
            params.append(self.eps0)
        emitter.uniaxialMaterial("ElasticPP", tag, *params)
...
class ElasticPPGap(UniaxialMaterial):
    """``uniaxialMaterial ElasticPPGap`` — elastic-perfectly-plastic gap.

    OpenSees command::

        uniaxialMaterial ElasticPPGap tag E Fy gap <eta> <damage>

    .. warning::
       The fork only *warns* on ``sign(Fy) != sign(gap)``
       (`EPPGapMaterial.cpp:109-111`) and then silently behaves
       according to ``sign(Fy)`` alone
       (``EPPGapMaterial::setTrialStrain``, `:147-168`). This primitive
    ...
    def __post_init__(self) -> None:
        if self.E <= 0:
            raise ValueError(
                f"ElasticPPGap: E must be > 0, got {self.E!r}"
            )
        if self.Fy == 0:
    ...
    def _emit(self, emitter: Emitter, tag: int) -> None:
        params: list[float | str] = [self.E, self.Fy, self.gap]
        if self.eta != 0.0 or self.damage:
            params.append(self.eta)
        if self.damage:
            params.append("damage")
        emitter.uniaxialMaterial("ElasticPPGap", tag, *params)
