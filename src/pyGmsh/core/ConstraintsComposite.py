...
# ---------------------------------------------------------------------------
# Imports from solvers/Constraints.py  (UNTOUCHED — all types live there)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Dispatch registry — replaces 12-branch isinstance chain
# ---------------------------------------------------------------------------
...
# Map Def type → ConstraintResolver method name
...
class ConstraintsComposite:
    ...
    def equal_dof(self, master_label, slave_label, *, master_entities=None,
        ...
        """Co-located nodes share selected DOFs. See :class:`EqualDOFDef`."""
    ...
    def rigid_link(self, master_label, slave_label, *, link_type="beam",
        ...
        """Rigid bar between master and slave. See :class:`RigidLinkDef`."""
    ...
    def penalty(self, master_label, slave_label, *, stiffness=1e10,
        ...
        """Soft spring between co-located node pairs. See :class:`PenaltyDef`."""
    ...
    def rigid_diaphragm(self, master_label, slave_label, *,
        ...
        """In-plane rigid body (e.g. floor diaphragm). See :class:`RigidDiaphragmDef`."""
    ...
    def rigid_body(self, master_label, slave_label, *,
        ...
        """All 6 DOFs of slave follow master rigidly. See :class:`RigidBodyDef`."""
    ...
    def kinematic_coupling(self, master_label, slave_label, *,
        ...
        """Generalised coupling: user picks constrained DOFs. See :class:`KinematicCouplingDef`."""
    ...
    def tie(self, master_label, slave_label, *, master_entities=None,
            ...
            name=None) -> TieDef:
        """Surface tie via shape function interpolation. See :class:`TieDef`."""
    ...
    def distributing_coupling(self, master_label, slave_label, *,
        ...
        """Load at master distributed to slave surface. See :class:`DistributingCouplingDef`."""
    ...
    def embedded(self, host_label, embedded_label, *, tolerance=1.0,
        ...
        """Embedded element: nodes follow host field. See :class:`EmbeddedDef`."""
    ...
    def tied_contact(self, master_label, slave_label, *,
        ...
        """Full surface-to-surface tie (bidirectional). See :class:`TiedContactDef`."""
    ...
    def mortar(self, master_label, slave_label, *,
               ...
               name=None) -> MortarDef:
        """Mortar coupling with Lagrange multiplier space. See :class:`MortarDef`."""
    ...
    def resolve(
        ...
        connectivity=None,
        *,
        node_map: dict[str, set[int]] | None = None,
        face_map: dict[str, np.ndarray] | None = None,
    ) -> list[ConstraintRecord]:
        """Resolve all stored definitions into concrete constraint records.

        Parameters
        ----------
        node_tags, node_coords : array-like
            Mesh data (from ``fem.node_ids``, ``fem.node_coords``).
        elem_tags, connectivity : array-like, optional
            Element data (needed for Level 3+ constraints).
        node_map : dict, optional
            ``{label: {node_tag, ...}}``.  From ``g.parts.build_node_map()``.
            ``None`` skips label-based lookups — use for single-body models
            that define constraints via entity-filter kwargs instead.
        face_map : dict, optional
            ``{label: face_connectivity_array}``.  From
            ``g.parts.build_face_map()``.  Required for surface constraints
            (tie, tied_contact, mortar).  A warning is emitted if a surface
            constraint is defined but face_map is None.
        """
        # Check for surface constraints without face_map
        ...
        if has_face_constraints and face_map is None:
            warnings.warn(
                "Surface constraints defined (tie/tied_contact/mortar) "
                "but face_map=None.  Pass g.parts.build_face_map() "
                "or these constraints will be skipped.",
                stacklevel=2,
            )
    ...
    def _resolve_nodes(
        ...
        """Shared node resolution: entity filter → label lookup → fallback."""
        import gmsh
    ...
    def _resolve_faces(
        ...
        role: str,
        defn: ConstraintDef,
        face_map: dict[str, np.ndarray],
    ) -> np.ndarray:
        """Shared face resolution: entity filter → label lookup → empty."""
        selected = getattr(defn, f"{role}_entities", None)
        if selected:
            parts = getattr(self._parent, "parts", None)
    ...
    def _resolve_node_pair(self, resolver, defn, node_map, face_map, all_nodes):
        """EqualDOF, RigidLink, Penalty — node-to-node pairing."""
    ...
    def _resolve_face_both(self, resolver, defn, node_map, face_map, all_nodes):
        """TiedContact, Mortar — both sides need faces + nodes."""
        m_faces = self._resolve_faces(defn.master_label, "master", defn, face_map)
    ...
    def list_defs(self) -> list[dict]:
        """Return constraint definitions as a list of summary dicts."""
        return [
    ...
    def list_records(self) -> list[dict]:
        """Return resolved records as a list of summary dicts."""
        out = []
    ...
    def clear(self) -> None:
        """Clear all definitions and resolved records."""
