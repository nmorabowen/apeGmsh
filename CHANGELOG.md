...
A **global** `ops.damping.rayleigh(...)` declared outside any stage was emitted in the flat (single-process) deck but **silently absent** from the partitioned (OpenSeesMP) deck — zero `rayleigh` lines — so an `np>1` run came out **undamped** (a plane-wave absorbing-boundary model showed a uniform ~14 % / max 45 % seq↔np4 surface discrepancy; the plane-wave handoff's load-bearing finding #1). Root cause: `apeSees._emit_partitioned` never called the global damping emitters that `_emit_flat` runs driver-post — stage-bound damping survived (re-emitted per stage by `_emit_stages_partitioned`), but the bridge's *global* `rayleigh_records` were dropped. The fix adds `_emit_global_damping_partitioned`, which emits a bare global `rayleigh αM βK βK0 βKc` **once outside any `partition_open` block** (every rank applies it to its local domain — the correct OpenSeesMP behaviour, parity with the flat path), for both staged and non-staged partitioned decks. The three forms that need per-rank element-tag routing / a parallel eigensolver the partitioned path does not yet wire — **region-scoped** Rayleigh (`on=`), the **Damping-object** attach (`-damp`), and **modal** damping (`eigen`/`modalDamping`) — now **fail loud** (`BridgeError`) under partitioned emit rather than silently dropping (the same class of bug); declare them stage-bound (`s.damping.*`) or emit single-process. Locked by `tests/opensees/integration/test_emit_partitioned_global_damping.py` (5: global rayleigh emits once outside the rank blocks on both Tcl + Py; the staged finding-#1 scenario; the three fail-loud guards). Full `tests/opensees` suite green (4966 passed); ruff (opensees hard gate) + mypy clean.
