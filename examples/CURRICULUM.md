...
| 07 | Load Patterns and Combinations | **NEW** `examples/EOS Examples/curriculum/02-building-blocks/07_load_patterns/07_load_patterns.ipynb` | Multiple named patterns (`dead`, `live`, `seismic`), `g.loads.case()` context manager, combination at analysis time. | `g.loads.case()`, multiple `timeSeries`, `LoadPattern` separation in OpenSees | static linear (superposed) | Pattern-wise displacements sum to the combined result within 1e-10. | 01 |
